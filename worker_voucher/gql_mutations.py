...
from worker_voucher.services import (
    WorkerVoucherService,
    GroupOfWorkerService,
    validate_acquire_unassigned_vouchers,
    validate_acquire_assigned_vouchers,
    validate_assign_vouchers,
    create_assigned_voucher,
    create_voucher_bill,
    create_unassigned_voucher,
    assign_voucher,
    economic_unit_user_filter,
    check_existing_active_vouchers,
    VoucherFormDraftService,
    validate_unassign_vouchers,
    unassign_voucher,
)
...
class CreateWorkerMutation(CreateInsureeMutation):
    ...
    def async_mutate(cls, user, **data):
        user_policyholders = PolicyHolder.objects.filter(economic_unit_user_filter(user)).values_list("id", flat=True)
        ...
        if ph.id not in user_policyholders:
            return [{"message": _("workers.validation.no_authority_to_use_selected_economic_unit")}]
...
class DeleteWorkerMutation(BaseMutation):
    ...
    def _validate_mutation(cls, user, **data):
        if (
            ...
            or not user.id
            or not user.has_perms(InsureeConfig.gql_mutation_delete_insurees_perms)
        ):
    ...
    def _mutate(cls, user, uuid=None, uuids=None, economic_unit_code=None, **data):
        ...
        eu_uuid = (
            PolicyHolder.objects.filter(economic_unit_user_filter(user), code=economic_unit_code)
...
class DeleteWorkerVoucherMutation(BaseMutation):
    ...
    def _validate_mutation(cls, user, **data):
        if (
            type(user) is AnonymousUser
            or not user.id
            or not user.has_perms(WorkerVoucherConfig.gql_worker_voucher_delete_perms)
        ):
            raise ValidationError("mutation.authentication_required")
...
class AcquireUnassignedVouchersMutation(BaseMutation):
    ...
    def _validate_mutation(cls, user, **data):
        ...
        if (
            type(user) is AnonymousUser
            or not user.id
            or not user.has_perms(WorkerVoucherConfig.gql_worker_voucher_acquire_unassigned_perms)
        ):
            raise ValidationError("mutation.authentication_required")
    ...
    def _mutate(cls, user, count=None, economic_unit_code=None, **data):
        ...
        with transaction.atomic():
            ...
            mutation = MutationLog.objects.get(
                client_mutation_id=client_mutation_id, client_mutation_label=client_mutation_label
            )
...
class AcquireAssignedVouchersMutation(BaseMutation):
    ...
    def _validate_mutation(cls, user, **data):
        if (
            type(user) is AnonymousUser
            or not user.id
            or not user.has_perms(WorkerVoucherConfig.gql_worker_voucher_acquire_assigned_perms)
        ):
            raise ValidationError("mutation.authentication_required")
...
class UnassignVouchersMutation(BaseMutation):
    ...
    def _validate_mutation(
        cls, user, economic_unit_code: int, id_: str | None = None, ids: list[str] | None = None, **_
    ):
        try:
...
class CreateOrUpdateGroupOfWorkerMutation(BaseMutation):
    ...
    def _mutate(cls, user, **data):
        try:
            ...
            eu_uuid = (
                PolicyHolder.objects.filter(economic_unit_user_filter(user), code=eu_code)
                .values_list("uuid", flat=True)
                .first()
            )
            if not eu_uuid:
            ...
            return [
                {"message": "worker_voucher.mutation.failed_to_create_or_update_group_of_worker", "detail": str(exc)}
            ]
...
class DeleteGroupOfWorkerMutation(BaseMutation):
    ...
    def _validate_mutation(cls, user, **data):
        if (
            type(user) is AnonymousUser
            or not user.id
            or not user.has_perms(WorkerVoucherConfig.gql_group_of_worker_delete_perms)
        ):
            raise ValidationError("mutation.authentication_required")
...
class CreateOrUpdateVoucherFormDraftMutation(BaseMutation):
    ...
    def _mutate(cls, user, **data):
        try:
            ...
            return [
                {"message": "worker_voucher.mutation.failed_to_create_or_update_voucher_form_draft", "detail": str(exc)}
            ]
